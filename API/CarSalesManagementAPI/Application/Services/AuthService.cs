...
public class AuthService : IAuthService
{
    ...
    public async Task<ApiResponse<LoginResponseDto>> GetUserInfo(int userId)
    {
        ...
        {
            _logger.LogError(ex, "Error retrieving user information for ID: {UserId}", userId);
            return new ApiResponse<LoginResponseDto>
            {
                Success = false,
    ...
    private static bool ValidatePassword(string providedPassword, string storedHash)
    {
        if (providedPassword == storedHash)
            return true;

        var computedHash = ComputeHash(providedPassword);
